...
export default function ProductCard({ name, slug, price, originalPrice, dosage, discount, category, categorySlug, inStock, images }: ProductProps) {
  ...
  return (
    <div className="block h-full relative group">
        <Link href={`/medicines/${slug}`} className="absolute inset-0 z-0" aria-label={`View ${name}`} />
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-lg transition-all duration-300 p-4 flex flex-col h-full relative z-10 pointer-events-none">
            ...
            <div className="mt-auto flex items-center justify-between pointer-events-auto">
                ...
                disabled={!inStock}
                >
                {inStock ? <Plus size={20} strokeWidth={2.5} /> : <X size={20} strokeWidth={2.5} />}
                </button>
            </div>
            </div>
        </div>
