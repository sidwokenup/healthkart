...
// SEO Generator Logic (Duplicated here to avoid TS compilation issues in this quick script)
...
const updatedProducts = products.map((p) => {
  // Check if category is Pain Relief (flexible matching)
  if (p.category && p.category.toLowerCase().includes("pain relief")) {
    // Only apply if it's NOT the detailed Ksalol entry we manually curated (optional safety check, but the generator checks for missing fields anyway)
    // The generator logic specifically checks `if (!field)` so existing fields are safe.
    updatedCount++;
...
console.log(
  `Successfully updated ${updatedCount} Pain Relief products with SEO fields.`
);
